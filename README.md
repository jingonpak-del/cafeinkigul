...
- **2단 레이아웃**: 왼쪽 = 인기글(매일 2·16시 수집·누적), 오른쪽 = 실시간 일반글(계속 축적)
  - 인기글은 네이버 갱신(1·15시) 직후인 2·16시에 수집. 그 시각에 서버가 꺼져 있었으면 켜질 때 보충 수집
  - 두 컬럼 모두 **글 작성시간 최신순** 정렬(여러 카페 섞여도 최신글이 위로)
  - **카페명** 열로 어느 카페 글인지 구분
- **WebSocket 실시간**: 새 글 뜨면 즉시 표시 + 토스트, 클릭 시 **본문·댓글 상세**
- 세션 만료·삭제글 자동 표시. 여러 사람이 각자 PC 브라우저로 동시 접속 (설치 불필요)
- 추적 카페 추가: `config/targets.json`의 `cafes`에 `{cluburl, club_id, name, boards}` 추가
