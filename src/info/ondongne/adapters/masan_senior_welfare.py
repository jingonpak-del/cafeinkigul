...
class MasanSeniorWelfareAdapter(AdapterBase):
    """Parser for 마산회원노인종합복지관 Gnuboard notices."""

    parser_version = "masan_senior_welfare_v1"
    base = "https://www.mhswc.or.kr"
    boards = [("공지사항", "https://www.mhswc.or.kr/bbs/board.php?bo_table=05_01")]
    include_keywords = ["모집", "참여", "신청", "교육", "프로그램", "상담", "강좌", "교실", "행사", "운동회", "쿠킹", "디지털"]
    negative_keywords = ["채용", "입찰", "계약", "공사", "휴관", "식단", "결산", "추경", "합격", "서류", "실습생", "강사 모집"]
    ...
    def parse_list_html(self, html: str, board_name: str = "공지사항", board_url: str | None = None) -> list[ListingItem]:
        ...
        for row in re.findall(r"<tr[^>]*>(.*?)</tr>", html or "", re.S | re.I):
            href = first_match(r'<a[^>]+href=["\']([^"\']*bo_table=05_01[^"\']*wr_id=\d+[^"\']*)["\']', row)
            if not href:
                continue
        ...
        if not items:
            for href, inner in re.findall(r'<a[^>]+href=["\']([^"\']*bo_table=05_01[^"\']*wr_id=\d+[^"\']*)["\'][^>]*>(.*?)</a>', html or "", re.S | re.I):
    ...
    def parse_detail_html(self, html: str, url: str, fallback: ListingItem | None = None) -> Event:
        ...
        event_rng = extract_labeled_range(body_text, ["교육기간", "운영기간", "행사기간", "일시", "기간"])
        ...
        return Event(
            ...
            target_audience=target or classify_audience(text) or "창원시 어르신", location_name=location or "마산회원노인종합복지관",
            ...
            tags=[t for t in [category, "창원시", "마산회원", "노인복지"] if t], parser_version=self.parser_version,
        ).finalize()
